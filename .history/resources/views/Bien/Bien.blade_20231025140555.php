@extends('layout')

@section('contenu')

    <div class="container">

        <div class="titre">
            <h1>Bien</h1>
            <p><a href="/bien" class="btn btn-outline-success">Afficher la liste</a></p>
        </div>

        <div class="mt-4">

            <div class="card mb-3" style="max-width: 60%;">
                <div class="row g-0">

                    <div class="image">
                        @if ($bien->photo === null || $bien->photo === "pas-de-photo.jpg")

                            <img src={{ asset('storage/images/unnamed.jpg') }} class="img rounded-start" style="width: 300px;
                            aspect-ratio: auto 1 / 1" alt="">
                        @else
                        <img src={{ asset('storage/images/'.$bien->photo) }} class="img rounded-start" alt="" style="width: 300px;
                        aspect-ratio: auto 1 / 1" alt="">
                        @endif
                    </div>
                    <div class="col-md-8">
                        <div class="card-body">

                            @if ($bien->statut == '0')
                                <p class="card-text mb-2" >
                                    <span class="badge rounded-pill text-bg-danger">vide</span>
                                </p>
                            @endif
                            @if ($bien->statut == '1')
                                <p class="card-text">
                                    <span class="badge rounded-pill text-bg-success">occupe</span>
                                </p>
                            @endif

                            <h2>
                                <span class="fw-bold">Adresse :</span>
                                {{ $bien->numrue }}, {{ $bien->nomrue }}
                            </h2>
                            
                            @if ($bien->numappartement)
                                <h2><span class="fw-bold">Numero appartement : </span>{{ $bien->numappartement }}</h2>
                            @endif

                            <h2><span class="fw-bold">Quartier : </span>{{ $bien->quartier }}</h2>
                            <h2><span class="fw-bold">Ville :</span> {{ $bien->ville }}</h2>
                            <h2><span class="fw-bold">Type de bien:</span> {{ $bien->typede_Bien->type }} </h2>

                            <p class="card-text">le loyer est de {{ $bien->loyer }}$</p>

                        </div>

                    </div>
                </div>
            </div>

        </div>
    </div>




@endsection
