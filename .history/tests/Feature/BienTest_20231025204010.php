...
class BienTest extends TestCase
{
    ...
    public function test_Modifier_le_crayon_de_la_base_de_donnees(){
        // Crée un crayon que vous souhaitez mettre à jour
        ...
        // Nouvelles données pour la mise à jour du crayon
        $nouvellesDonnees = [
            'type' => 'Crayon mis à jour',
        ...
        // Effectue une requête HTTP POST pour mettre à jour le crayon avec les nouvelles données
        $response = $this->put('crayons/' . $crayon->id, $nouvellesDonnees);

        // Vérifie si la réponse a un code de statut HTTP 302 (redirection)
        $response->assertStatus(302);

        // Vérifie si la réponse redirige vers la route de détails du crayon mis à jour
        $response->assertRedirect('crayons');

        // Recharge le crayon depuis la base de données
        $crayonMisAJour = Crayon::find($crayon->id);

        // Vérifie si les propriétés du crayon ont été mises à jour en base de données
        $this->assertEquals($nouvellesDonnees['type'], $crayonMisAJour->type);
        $this->assertEquals($nouvellesDonnees['marque'], $crayonMisAJour->marque);
        $this->assertEquals($nouvellesDonnees['couleur'], $crayonMisAJour->couleur);
        $this->assertEquals($nouvellesDonnees['quantite'], $crayonMisAJour->quantite);
        $this->assertEquals($nouvellesDonnees['prix'], $crayonMisAJour->prix);
    }
