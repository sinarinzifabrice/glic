...
class BienTest extends TestCase
{
    ...
    public function test_Modifier_le_bien_de_la_base_de_donnees(){
        // Crée un bien que vous souhaitez mettre à jour
        $bien = Bien::create([
            'loyer' => 1200,
            'numappartement' =>7,
            'numrue' => 23,
        ...
        // Nouvelles données pour la mise à jour du bien
        $nouvellesDonnees = [
            'loyer' => 34500,
            'numappartement' =>5,
            'numrue' => 2,
        ...
        // Effectue une requête HTTP POST pour mettre à jour le bien avec les nouvelles données
        $response = $this->put('biens/' . $bien->id, $nouvellesDonnees);

        // Recharge le bien depuis la base de données
        $bienMisAJour = Bien::find($bien->id);

        // Vérifie si les propriétés du bien ont été mises à jour en base de données
        $this->assertEquals($nouvellesDonnees['loyer'], $bienMisAJour->loyer);
        $this->assertEquals($nouvellesDonnees['numappartement'], $bienMisAJour->numappartement);
        $this->assertEquals($nouvellesDonnees['numrue'], $bienMisAJour->numrue);
        $this->assertEquals($nouvellesDonnees['nomrue'], $bienMisAJour->nomrue);
        $this->assertEquals($nouvellesDonnees['ville'], $bienMisAJour->ville);
        $this->assertEquals($nouvellesDonnees['quartier'], $bienMisAJour->quartier);
        $this->assertEquals($nouvellesDonnees['typede_bien'], $bienMisAJour->typede_bien);
        $this->assertEquals($nouvellesDonnees['statut'], $bienMisAJour->statut);
        $this->assertEquals($nouvellesDonnees['photo'], $bienMisAJour->photo);
