...
class BienTest extends TestCase
{
    ...
    public function test_Creer_un_crayon_avec_toutes_les_valeur_fournies(){

        // Crée un tableau associatif contenant les détails du crayon à créer
        $crayon = [
            'type' => 'Crayon de test',
            'marque' => 'Test Marque',
        ...
        // Effectue une requête HTTP POST pour créer un crayon avec les détails fournis
        $response = $this->post('crayons',$crayon);

        // Vérifie si la réponse a un code de statut HTTP 302 (redirection)
        $response->assertStatus(302);

        // Vérifie si la réponse redirige vers la route 'crayons'
        $response->assertRedirect('crayons');

        // Vérifie si les données du crayon existent dans la base de données
        $this->assertDatabaseHas('crayons', $crayon);

        // Récupère le dernier crayon de la base de données
        $derniercrayon = Crayon::latest()->first();

        // Vérifie si les propriétés du dernier crayon correspondent aux détails fournis
        $this->assertEquals($crayon['type'], $derniercrayon->type);
        $this->assertEquals($crayon['prix'], $derniercrayon->prix);
