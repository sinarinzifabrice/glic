...
//     // Vérifie si la réponse redirige vers la route 'bien.index' (ou une autre route appropriée après la suppression)
//     $response->assertRedirect('bien.index'); // Assurez-vous que la route est correcte
...
// }
    ...
    public function test_Supprimer_le_bien_de_la_base_de_donnees()
    {
        ...
        // Effectue une requête HTTP DELETE pour supprimer le bien
        $response = $this->delete(route('bien.destroy', ['bien' => $bien]));

        // Vérifie si la réponse a un code de statut HTTP 302 (redirection)
        $response->assertStatus(302);

        // Vérifie si la réponse redirige vers la route 'bien.index' (ou une autre route appropriée après la suppression)
        $response->assertRedirect('bien'); // Assurez-vous que la route est correcte
